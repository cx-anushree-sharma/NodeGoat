...
def load_config(config_path):
    ...
    if config_path:
        ...
        if resolved_config_path != base_dir and not resolved_config_path.startswith(base_dir + os.sep):
...
def check_source_path(cfg):
    ...
    # Containment check - see the note in load_config() on why the trailing
    # separator matters and why normalization alone is not validation.
    if src_path != base_dir and not src_path.startswith(base_dir + os.sep):
